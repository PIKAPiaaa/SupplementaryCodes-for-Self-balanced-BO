...
#%% Gain/loss sublattice dynamics figure


# Time axis
x = np.asarray(t_eval, dtype=float)

# Zero-based Python indexing is used in our simulation:
# odd array indices  -> loss sublattice (-i*gamma)
# even array indices -> gain sublattice (+i*gamma)
# In one-based physical site numbering, these correspond to
# even- and odd-numbered sites, respectively.
loss_rows = np.abs(sol1.y[1::2, :])
gain_rows = np.abs(sol1.y[::2, :])
...
X_loss, Y_loss = np.meshgrid(x, y_loss)
X_gain, Y_gain = np.meshgrid(x, y_gain)
...
# Bottom: gain sublattice
ax[1].pcolormesh(
    X_gain, Y_gain, gain_rows,
    cmap="viridis",
    shading="auto",
    vmin=vmin,
    vmax=vmax * 1.5
