...
time_labels = [r"$t=0$", r"$T/2$", r"$T$", r"$3T/2$"]


time_ids = [np.argmin(np.abs(Tarr - tt)) for tt in target_times]

# Lattice window to display
n_min, n_max = 80, 120
sites = np.arange(n_min, n_max + 1)


odd_sites = sites[sites % 2 == 1]
even_sites = sites[sites % 2 == 0]
...
fig, axes = plt.subplots(2, 2, figsize=(9.0, 4.8), dpi=150)
axes = axes.ravel()

for ax, tid, lab in zip(axes, time_ids, time_labels):
    amp = A[:, tid]

    amp_odd = amp[odd_sites]
    amp_even = amp[even_sites]

    # odd: blue
    ax.vlines(odd_sites, 0, amp_odd, color="#1f4ea8", lw=1.1)
    ax.scatter(
        odd_sites, amp_odd,
        s=42, facecolors="#8ea0c8", edgecolors="#1f4ea8",
        linewidths=1.2, zorder=3
    )

    # even: red
    ax.vlines(even_sites, 0, amp_even, color="red", lw=1.1)
    ax.scatter(
        even_sites, amp_even,
        s=42, facecolors="#d8a3a3", edgecolors="red",
    ...
    ax.set_ylim(0, 1.08 * np.max(A[sites][:, time_ids]))
    ax.set_xticks([])
    ax.set_yticks([])
...
legend_handles = [
    Line2D([0], [0], marker='o', linestyle='None',
           markerfacecolor="#8ea0c8", markeredgecolor="#1f4ea8",
           markeredgewidth=1.2, markersize=7, label="odd"),
    Line2D([0], [0], marker='o', linestyle='None',
           markerfacecolor="#d8a3a3", markeredgecolor="red",
           markeredgewidth=1.1, markersize=7, label="even"),
]
axes[1].legend(
    handles=legend_handles,
    frameon=False, fontsize=20,
    loc="upper right", ncol=2,
    handlelength=0.0, columnspacing=1.4
)

plt.subplots_adjust(left=0.05, right=0.98, bottom=0.10, top=0.95, wspace=0.16, hspace=0.38)
plt.show()