...
sol1= solve_ivp(f, [0,4], zpump, t_eval=t_eval,max_step=0.01)
...
# ==============================
# total population curve overlay
# ==============================
...
# map population curve to the vertical range of the upper heatmap
pop_y0 = 54
pop_height = 24
...
if su_max > su_min:
    pop_curve = pop_y0 + pop_height * (su - su_min) / (su_max - su_min)
else:
    pop_curve = pop_y0 + pop_height * np.ones_like(su)
...
print("Population min =", su.min())
print("Population max =", su.max())
