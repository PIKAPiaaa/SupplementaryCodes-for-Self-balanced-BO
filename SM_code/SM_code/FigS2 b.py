...
sol1= solve_ivp(f, [0,4], zpump, t_eval=t_eval,max_step=0.02)
...
vmin = 0.0
vmax = np.percentile(Z[50:150,:], 99.7)

# vertical guide lines
# vertical guide lines: first local maximum of C, second local minimum of C
guide_centers = [0.5, 3.5] 
guide_window = 0.15

def find_local_extremum_time(T, C, center, window, mode="max"):
    mask = (T >= center - window) & (T <= center + window)
    if not np.any(mask):
        raise ValueError(f"No time points found around center={center}")

    if mode == "max":
        idx_local = np.argmax(C[mask])
    elif mode == "min":
        idx_local = np.argmin(C[mask])
    else:
...
t_guides = [
    find_local_extremum_time(T, C, 0.5, guide_window, mode="max"),
    find_local_extremum_time(T, C, 3.5, guide_window, mode="min"),
]
