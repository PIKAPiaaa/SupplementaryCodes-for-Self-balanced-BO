...
# Construct a loss-sublattice-polarized initial state.
# The antisymmetric superposition of the wave packets centered at
# k = pi/2 and k = 3*pi/2 cancels the amplitudes on the even array
# indices, and leaves nonzero amplitudes only on the odd array indices.
# In the zero-based numerical basis, the odd array indices correspond
# to the loss sublattice carrying the on-site potential -i*gamma.
zpump=1j*np.array(zpump0+bi*Zpump0,dtype=complex)
...
ax2 = ax1.twinx()
ax2.plot(T, su, color="#d8cf00", lw=1.2, zorder=6)
ax2.set_ylim(0, 5)
